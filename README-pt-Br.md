...
Todo software é construído em uma stack de tecnologias, e você precisa entender o suficiente dessa stack para construir seu app. A razão para o ecossistema de ferramentas do React parecer esmagadora é porque é sempre explicada na ordem errada.
...
Adicionalmente, têm alguns tópicos que são frequentemente mencionados na comunidade do React que são "muito avançados". Os tópicos abaixo são interessantes, mas eles são difíceis de entender, e são, de longe, menos populares que os tópicos acima e **não são requeridos para a maioria dos apps**.
...
Exemplos de empacotadores JavaScript são `webpack` e `browserify`. Ambos são boas escolhas, mas eu prefiro `webpack` pois possui funcionalidades que fazem o desenvolvimento muito mais fácil. Mesmo a documentação sendo confusa, eu tenho um [template plug-and-play para começar](https://github.com/petehunt/react-webpack-template) e eu escrevi um guia [como-fazer](https://github.com/petehunt/webpack-howto) para casos mais complexos.

React agora oferece uma ferramenta oficial chamada [Create React App](https://github.com/facebookincubator/create-react-app). Ela permite criar projetos usando React e Webpack sem nenhuma configuração. Ela possui algumas limitações, mas é um bom ponto de partida e recebe atualizações todo tempo. Ela também oferece um recurso de "ejeção" que copia todas as configurações e dependências no seu projeto para você ter controle total sobre ele.

Uma coisa para manter em mente: `CommonJS` usa a função `require()` para importar módulos, então muitas pessoas ficam confusas e pensam que tem algo haver com o projeto chamado `require.js`. Por um bom número de razões técnicas, eu gostaria de sugerir que você evitasse `require.js`. Ele também não é muito popular no ecossistema do React.
...
Você provavelmente já ouviu falar sobre Flux. Existe uma *tonelada* de mal entendidos sobre Flux por aí.

Muita gente começa a construir um app e gostaria de definir seu próprio modelo de dados, e pensam que precisam usar Flux para fazer isso. **Essa é uma forma errada de adotar o Flux. O Flux só deveria ser adicionado quando muitos componentes já foram cosntruídos.**

Os componentes React são arranjados em hierarquia. A maioria das vezes, seu modelo de dados também segue uma hierarquia. Nessa situação o Flux te adiciona muita coisa. As vezes não há hierarquia no seu modelo de dados. Quando seus componentes React começam a receber `props` (propriedades) que parecem estranhas, ou você possui um pequeno número de componentes começando a ficar muito complexos, então talvez você queira dar uma olhada no Flux.

**Você saberá quando precisará do Flux. Se você não está certo se precisa disso, você realmente não precisa disso.**

Se você decidiu usar Flux, a biblioteca mais popular e melhor documentada é [Redux](http://redux.js.org/). Há *muitas* outras alternativas por aí, e você ficará tentado a avaliar a maioria delas, mas meu conselho é para você ficar apenas no mais popular.

## Aprendendo inline styles

Antes do React, muita gente reutilizava estilos de CSS com complicados processadores de style sheets (folhas de estilo) como SASS. Como o React torna fácil a escrita de componentes reutilizáveis, sua folha de estilo pode ser menos complicada. A maioria da comunidade (me incluindo) estão experimentando se livrar de folhas de estilos "altogether" (tudo em uma só).

Essa é uma idéia bastante louca por bom número de razões. Faz com que media queries fiquem mais difíceis, e é possível que tenha uma limitação na performance ao usar essa técnica. **Enquanto estiver começando com React, apenas crie os estilos da maneira como você normalmente faria.**
...
Alternativamente, você também pode usar [Módulos CSS](http://glenmaddern.com/articles/css-modules), mais especificamente [react-css-modules](https://github.com/gajus/react-css-modules). Com módulos CSS você ainda escreverá CSS (ou SASS/LESS/Stylus), mas você pode gerenciar e fazer seus arquivos CSS como você faria com "inline styles" no React. E você não precisará se preocupar em gerenciar o nom de de classes usando metodologias como BEM, pois isso será feito por baixo dos panos pelo sistema de módulos.
...
Você apenas precisa renderizar no servidor se você perceber que a renderização inicial está muito lenta ou se você você quer melhorar o sistema de ranking de busca. Enquando for verdade que o Google agora indexa conteúdo renderizado no cliente, a partir de janeiro de 2016 todo tempo é mensurado e o ranking sofre um efeito negativo, potencialmente por causa da penalidade na renderização do lado do cliente.

Renderização no lado do servidor ainda requer uma grande quantidade de ferramentas para funcionar direito. Já que transparentemente suporta componentes React escritos sem foco na renderização no lado do servidor, você deveria primeiramente construir seu app e se preocupar com a renderização do lado do servidor mais tarde. Você não precisa reescrever todos os seus componentes para suportar isso.

## Aprendendo Immutable.js

[Immutable.js](https://facebook.github.io/immutable-js/) disponibiliza uma série de estruturas de dados que podem ajudar a resolver certos problemas de performance na construção de apps em React. É uma excelente biblioteca, e você provavelmente irá usar muito quando seus apps forem mais complexos, mas é completamente desnecessário até que você tenha uma apreciação em implementações performáticas.
